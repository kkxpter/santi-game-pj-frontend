  ...
  useEffect(() => {
    const loadFromLocalStorage = () => {
      try {
        ...
        const storedUserStr = localStorage.getItem('santi_user');

        const savedStats = savedStatsStr ? JSON.parse(savedStatsStr) : {};
        const storedUser = storedUserStr ? JSON.parse(storedUserStr) : null;

        // บรรทัดนี้คือจุดที่ Error: เราต้องใส่คอมเมนต์นี้เพื่อบอกระบบว่า "เรารู้ตัวว่าทำอะไรอยู่ ยอมให้ทำเถอะ"
        ...
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setIsLoaded(true); // ยังไงก็ต้องเปิดหน้าเว็บให้ติด
      }
    };
